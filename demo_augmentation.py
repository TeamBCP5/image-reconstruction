...
if __name__ == '__main__':
    ...
    pix2pix_aug_dir = os.path.join(args.save_dir, 'pix2pix') # Pix2Pix augmentation 이미지가 저장될 디렉토리 경로
    hinet_aug_dir = os.path.join(args.save_dir, 'hinet') # HINet augmentation 이미지가 저장될 디렉토리 경로
    ...
    for input_path in tqdm(train_input_paths, desc='[Sample Augmentation]'):
        ...
        for i in range(6):
            pix2pix_aug = demo_augmentation_pix2pix(probs=[1.0 if k == i else 0.0 for k in range(6)])
            aug_output = pix2pix_aug(image=image)['image']
            cv2.imwrite(os.path.join(pix2pix_aug_dir, f'{img_name}_{i}.png'), aug_output)
        
        for i in range(2):
            hinet_aug = demo_augmentation_hinet(probs=[1.0 if k == i else 0.0 for k in range(2)])
