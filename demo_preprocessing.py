...
if __name__ == '__main__':
    ...
    parser.add_argument("--patch_size", type=int, default=512, help='Sliding Window 시 사용할 patch 사이즈')
    ...
    pix2pix_preprocessing_dir = os.path.join(args.save_dir, 'pix2pix') # Pix2Pix 전처리(sliding window) 결과가 저장될 디렉토리 경로
    hinet_preprocessing_dir = os.path.join(args.save_dir, 'hinet') # HINet 전처리(resize) 결과가 저장될 디렉토리 경로
    ...
    train_label_paths = sorted(glob(os.path.join(args.data_dir, 'train_label_img', '*')))[:args.num_samples]

    # Pix2Pix
    cut_img(train_input_paths, train_label_paths, save_dir=pix2pix_preprocessing_dir, stride=args.stride, patch_size=args.patch_size)

    # HINet
    for input_path, label_path in tqdm(zip(train_input_paths, train_label_paths), '[Resize]'):
        input_img = cv2.imread(input_path)
        input_name = os.path.basename(input_path).split('.png')[0]
        label_img = cv2.imread(label_path)
        label_name = os.path.basename(label_path).split('.png')[0]

        cv2.imwrite(os.path.join(original_dir, 'train_input_img', f'{input_name}.png'), input_img)
        cv2.imwrite(os.path.join(original_dir, 'train_label_img', f'{label_name}.png'), label_img)

        input_img = A.Resize(1224, 1632, p=1.0)(image=input_img)['image']
        label_img = A.Resize(1224, 1632, p=1.0)(image=label_img)['image']

        cv2.imwrite(os.path.join(hinet_preprocessing_dir, 'train_input_img', f"{input_name}_resized.png"), input_img)
        cv2.imwrite(os.path.join(hinet_preprocessing_dir, 'train_label_img', f"{label_name}_resized.png"), label_img)
